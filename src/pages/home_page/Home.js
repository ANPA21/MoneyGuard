...
export const Home = () => {
  ...
  const { fetchTransactions, deleteItem } = require('redux/transactionsRedux/transactionsOperations');
  ...
  return (
    <HomeStyled>
      ...
      <ul className='transaction'> 
        {transactions.map(({createdAt, type, category, comment, value, _id}) => {
          return (
            <li key={_id} className='data'>
              ...
              <p >{comment}</p>
              <p >{value}</p>
              <button  onClick={() => {deleteTransactions(_id)}}>Delete</button>
            </li>
          );
        })}
      </ul>
